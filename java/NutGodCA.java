...
package kotori.koncclient.konc.module.modules.crystal;

import com.mojang.realmsclient.gui.ChatFormatting;
import me.zero.alpine.listener.EventHandler;
import me.zero.alpine.listener.Listener;
import kotori.koncclient.konc.command.Command;
import kotori.koncclient.konc.event.events.PacketEvent;
import kotori.koncclient.konc.event.events.RenderEvent;
import kotori.koncclient.konc.module.Module;
import kotori.koncclient.konc.module.ModuleManager;
import kotori.koncclient.konc.module.modules.chat.AutoEZ;
import kotori.koncclient.konc.setting.Setting;
import kotori.koncclient.konc.setting.Settings;
import kotori.koncclient.konc.util.EntityUtil;
import kotori.koncclient.konc.util.Friends;
import kotori.koncclient.konc.util.KamiTessellator;
...
@Module.Info(name = "NutGodCA", category = Module.Category.CRYSTAL)
public class NutGodCA extends Module {
    ...
    public void onUpdate() {
        ...
        if (lastTarget instanceof EntityPlayer && ModuleManager.getModuleByName("AutoEZ").isEnabled()) {
            kotori.koncclient.konc.module.modules.chat.AutoEZ AutoEZ = (AutoEZ)ModuleManager.getModuleByName("AutoEZ");
    ...
    public void onWorldRender(RenderEvent event) {
        if (render != null) {
            ...
            KamiTessellator.drawBox(render, r, g, b, 77, 63);
            KamiTessellator.release();
            KamiTessellator.prepare(7);
            KamiTessellator.drawBoundingBoxBlockPos(render, 1.00f,  r, g, b, 255);
            } else {
                KamiTessellator.prepare(7);
                KamiTessellator.drawBox(render, Red.getValue(), Green.getValue(), Blue.getValue(), 77, 63);
                KamiTessellator.release();
                KamiTessellator.prepare(7);
                KamiTessellator.drawBoundingBoxBlockPos(render, 1.00f, Red.getValue(), Green.getValue(), Blue.getValue(), 244);
            }
                KamiTessellator.release();
        }
    ...
    protected void onEnable() {
        ...
        if (alert.getValue() && mc.world != null) {
            Command.sendRawChatMessage("\u00A7NutGodCA ON");
        }
    }
    ...
    public void onDisable() {
        if (alert.getValue() && mc.world != null) {
            Command.sendRawChatMessage("\u00A7NutGodCA" + ChatFormatting.RED.toString() + "OFF");
        }
        render = null;
        resetRotation();
    }
